...
| | Swift | Python MLX |
| --- | --- | --- |
| **val_bpb** (lower = better) | **1.551** | 1.863 |
| Optimizer | **Muon + AdamW** | AdamW only |
| Startup to first step | **0.1s** | 1.4s |
| Runtime dependencies | **0** (single binary) | Python + MLX + NumPy |

Swift achieves **17% better val_bpb** on the same 5-minute budget. The difference comes from the Muon optimizer — it orthogonalizes gradients before each update, so the model learns more per step. The Python MLX port only has AdamW.
